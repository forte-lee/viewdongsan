...
import { useGetPropertyDeleteAllSiteAdmin } from "@/hooks/supabase/property/useGetPropertyDeleteAllSiteAdmin";
...
function SiteAdminDeletedPage() {
    const router = useRouter();
    const { propertyDeletes, getPropertyDeletesAll, isLoading } = useGetPropertyDeleteAllSiteAdmin();
    ...
    const isBulkRestoringRef = useRef(false);
    const { deletePropertyDeletesBulk } = useDeletePropertyDelete();
    const { restorePropertiesBulk } = useRestoreProperty();
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            const success = await deletePropertyDeletesBulk(ids);
            if (success) {
                setBulkSelectedIds(new Set());
    ...
    const handleBulkRestore = async () => {
        const ids = Array.from(bulkSelectedIds);
        if (ids.length === 0) return;
        if (isBulkRestoringRef.current) return;
        isBulkRestoringRef.current = true;
        setIsBulkRestoring(true);
        try {
            const properties = filteredProperties.filter((p) => bulkSelectedIds.has(p.id));
            const success = await restorePropertiesBulk(properties);
            if (success) {
                setBulkSelectedIds(new Set());
                getPropertyDeletesAll();
            }
        } finally {
            isBulkRestoringRef.current = false;
    ...
    useEffect(() => {
        if (propertyDeletes.length === 0 && !isLoading) {
            getPropertyDeletesAll();
        }
    }, [propertyDeletes, isLoading, getPropertyDeletesAll]);
    ...
    useEffect(() => {
        ...
        filter();
    }, [typeFilter, propertyDeletes, addressSearchKeyword, filterCompanyId, employees]);
...
export default SiteAdminDeletedPage;
