...
function SiteAdminPropertiesPage() {
    ...
    const { copyPropertiesBulk } = useCopyProperty();
    ...
    const handleBulkCopy = async () => {
        const ids = Array.from(bulkSelectedIds);
        if (ids.length === 0) return;
        if (isBulkCopying) return;
        setIsBulkCopying(true);
        try {
            const propertiesToCopy = filteredProperties.filter((p) => ids.includes(p.id));
            const success = await copyPropertiesBulk(propertiesToCopy);
            if (success) {
                setBulkSelectedIds(new Set());
                getPropertysAll();
            }
                        ...
                        {bulkSelectedIds.size > 0 && (
                            <>
                                ...
                                <Button
                                    variant="outline"
                                    size="sm"
                                    className="text-sm bg-purple-50 text-purple-700 hover:bg-purple-100 border-purple-200"
                                    onClick={handleBulkCopy}
                                    disabled={isBulkCopying}
                                >
                                    {isBulkCopying ? "복사 중..." : `복사 (${bulkSelectedIds.size}개)`}
                                </Button>
                                <AlertDialog>
                            ...
                            {visibleRows.map((row) => (
                                ...
                                >
                                    <AdminPropertyReadCard
                                        ...
                                        showBulkCheckbox
                                        isBulkSelected={bulkSelectedIds.has(row.property.id)}
                                        onBulkSelectChange={(checked) => toggleBulkSelect(row.property.id, checked)}
                                    />
                                    {!row.isReply && row.hasReplies && (
                                        <div className="flex justify-start px-4 pb-2">
                                            <Button
                                                variant="ghost"
                                                className="h-8 px-0 text-sm text-gray-600 hover:text-amber-600"
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    setExpandedGroupKeys((prev) => {
                                                        const next = new Set(prev);
