...
function MyListContent() {
    ...
    useEffect(() => {
        if (employeeIdParam && loggedInEmployeeId !== null) {
            ...
            if (!isNaN(paramId) && paramId !== loggedInEmployeeId) {
                ...
                router.replace(`/manage/mylist?employeeId=${loggedInEmployeeId}`);
            }
    ...
    // ✅ Supabase에서 전체 매물 가져오기 (Atom이 비어 있을 경우)
    useEffect(() => {
    ...
    useEffect(() => {
        const filter = () => {
            ...
            const filtered = propertysAll.filter((p) => {
                ...
                // 개인매물 필터: 체크가 ON(true)이면 관리처가 "개인매물"인 매물만 표시
                const matchPersonalProperty =
                    typeFilter.isPersonalProperty !== true ||
                    d.manager === "개인매물";

                const matchPhone =
                    !typeFilter.phoneKeyword ||
                    (Array.isArray(d.phones) &&
                        d.phones.some((p: string) =>
                            p.replace(/[^0-9]/g, "").includes(
                                typeFilter.phoneKeyword!.replace(/[^0-9]/g, "")
                            )
                        ));
        ...
        filter();
    }, [typeFilter, propertysAll, addressSearchKeyword, currentEmployeeId]);
