  ...
  const images =
    ...
    try {
      localStorage.setItem(
        `propertyMainDetail:${property_Data.id}`,
    ...
    const url = `/property-main-detail?id=${encodeURIComponent(
      String(property_Data.id)
    )}`;
    window.open(url, "_blank", "width=1300,height=1050,scrollbars=yes");
  };

  return (
    <div className="flex flex-col w-[100px] justify-center items-center relative group">
      {images.length > 0 ? (
        <img
          src={images[0]}
          alt="Property"
          className="w-[100px] h-[100px] object-cover rounded-md group-hover:scale-110 transition-transform duration-300 cursor-pointer"
        ...
        <Button
          variant="secondary"
          className="flex w-[100px] h-[100px] justify-center items-center text-gray-700"
