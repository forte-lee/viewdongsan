...
export const handleVillamulti = (property: Property, employees?: Employee[]): ShowData => {
    let title_data = "";
    let address = "";
    let area = "";
    let floor = "";
    ...
    // 금액 점수 계산 (공동주택인 경우)
    // handle 함수에서는 기본값으로 "월세"를 사용 (실제 선택은 PropertyCardDetailView에서)
    // ShowData의 score_price는 일단 undefined로 두고, PropertyCardDetailView에서 선택된 거래 유형에 따라 계산
    const priceScore = undefined; // calculatePriceScore(property.data, "월세");
