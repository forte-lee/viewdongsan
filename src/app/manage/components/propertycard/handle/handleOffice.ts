...
export const handleOffice = (property: Property, employees?: Employee[]): ShowData => {
    let title_data = "";
    let address = "";
    let area = "";
    let floor = "";
    ...
    if(property.data.type !== undefined){
        ...
        if (property.data.area_ground !== ""){
            area = "대지:" + property.data.area_ground + "㎡ ";
        }
        ...
        if (property.data.area_type !== ""){
            area += "타입:" + property.data.area_type + " ";
        }
