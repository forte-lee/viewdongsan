...
export const handleBuilding = (property: Property, employees?: Employee[]): ShowData => {
    let title_data = "";
    let address = "";
    let area = "";
    let floor = "";
    ...
    if (property.data.type !== undefined) {
        ...
        if (property.data.area_ground !== "") {
            ...
            if (!isNaN(ground) && ground > 0) {
                area += `대지:${raw}㎡(${(ground / 3.3).toFixed(0)}평)\n`;
            } else {
                area += `대지:${raw}㎡\n`;
            }
        }
        
        if (property.data.area_grossfloor !== "") {
            const raw = property.data.area_grossfloor;
            const gross = Number(raw.replace(/,/g, ""));
            if (!isNaN(gross) && gross > 0) {
                area += `연:${raw}㎡(${(gross / 3.3).toFixed(0)}평)`;
            } else {
                area += `연:${raw}㎡`;
            }
        }
        ...
        if (property.data.floor_top !== "") {
            floor = "층:" + property.data.floor_top;
        }

        if (property.data.floor_underground !== "") {
            floor += "/" + property.data.floor_underground;
        }
