...
export default function OfficeRegister() {
    ...
    const validateFields = (): string[] => {
        ...
        if (!state.trade_price && !state.trade_deposit && (!state.trade_rent || !state.trade_rent_deposit)) fields.push("거래금액");
        if (!state.admin_cost && !state.admin_cost_self) fields.push("관리비");
        if (!state.already_jobtype) fields.push("현재업종");
        if (!state.already_jobwant) fields.push("비선호업종");
        if (!state.already_premium) fields.push("권리금");
        ...
        if (!state.floor_applicable || !state.floor_top || !state.floor_underground) fields.push("층");
        if (!state.structure_room || !state.structure_bathroom) fields.push("구조");
        if (!state.parking_available) fields.push("주차가능여부");
        if (!state.parking_method) fields.push("주차방식");
            ...
            >
                ...
                {isSimpleMode ? (
                    ...
                    <>
                        ...
                        {/* 반려동물 */}
                        <WaterSection
                            water_possible={state.water_possible}
                            onWaterPossibleChange={(value) => setField("water_possible", value)}
                            water_memo={state.water_memo}
                            onWaterMemoChange={(value) => setField("water_memo", value)}
                        />
