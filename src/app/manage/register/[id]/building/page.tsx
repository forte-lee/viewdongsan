...
export default function BuildingRegister() {
    ...
    const validateFields = (): string[] => {
        ...
        if (!state.trade_price && !state.trade_deposit && (!state.trade_rent || !state.trade_rent_deposit)) fields.push("거래금액");
        if (!state.admin_cost && !state.admin_cost_self) fields.push("관리비");
        if (!state.building_total_tenant) fields.push("임차인수");
        if (!state.building_total_deposit) fields.push("총보증금");
        if (!state.building_total_rent) fields.push("총월세");
        if (!state.building_total_admincost) fields.push("총관리비");
        if (!state.enter_date && !state.enter_is_discuss && !state.enter_is_hasi && !state.enter_is_now) fields.push("입주예정일");
        if (!state.area_ground) fields.push("대지면적");
        if (!state.area_grossfloor) fields.push("연면적");
        if (!state.floor_top || !state.floor_underground) fields.push("층");
        if (!state.construction_standard || !state.construction_date) fields.push("건축물일자");
        if (!state.structure_room || !state.structure_bathroom) fields.push("구조");
        if (!state.parking_available) fields.push("주차가능여부");
        if (state.parking_method.length === 0) fields.push("주차방식");
        if (!state.parking_total) fields.push("총주차");
        if (!state.violation) fields.push("위반사항");
            ...
            >
                ...
                {isSimpleMode ? (
                    ...
                    <>
                        ...
                        {/* 기보증금 */}
                        <BuildingDepositSection
                            buildingTotalTenant={state.building_total_tenant}
                            onBuildingTotalTenantChange={(value) => setField("building_total_tenant", value)}
                            buildingTotalDeposit={state.building_total_deposit}
                            onBuildingTotalDepositChange={(value) => setField("building_total_deposit", value)}
                            buildingTotalRent={state.building_total_rent}
                            onBuildingTotalRentChange={(value) => setField("building_total_rent", value)}
                            buildingTotalAdmincost={state.building_total_admincost}
                            onBuildingTotalAdmincostChange={(value) => setField("building_total_admincost", value)}
                            buildingTotalCost={state.building_total_cost}
                            onBuildingTotalCostChange={(value) => setField("building_total_cost", value)}
                            buildingTotalRate={state.building_total_rate}
                            onBuildingTotalRateChange={(value) => setField("building_total_rate", value)}
                            buildingRooms={state.building_rooms}
                            onBuildingRoomsChange={(value) => setField("building_rooms", value)}
                            buildingDeposits={state.building_deposits}
                            onBuildingDepositsChange={(value) => setField("building_deposits", value)}
                            buildingRents={state.building_rents}
                            onBuildingRentsChange={(value) => setField("building_rents", value)}
                            buildingAdmincosts={state.building_admincosts}
                            onBuildingAdmincostsChange={(value) => setField("building_admincosts", value)}
                            buildingMemos={state.building_memos}
                            onBuildingMemosChange={(value) => setField("building_memos", value)}
                            buildingEndDates={state.building_enddates}
                            onBuildingEndDatesChange={(value) => setField("building_enddates", value)}
                            buildingJobs={state.building_jobs}
                            onBuildingJobsChange={(value) => setField("building_jobs", value)}
                        />
                        ...
                        <InteriorSection
                            ...
                            interior_memo={state.interior_memo}
                            onInteriorChange={(value) => setField("interior", value)}
                            onInteriorMemoChange={(value) => setField("interior_memo", value)}
                        />
