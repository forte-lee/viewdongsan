...
interface GuestRegisterHeaderProps {
    handleSubmit: () => Promise<void>; // ✅ handleSubmit이 Promise를 반환하도록 변경
    guestId: number; // 손님 ID
}

function GuestRegisterHeader({ handleSubmit, guestId}: GuestRegisterHeaderProps) {
        ...
        try {
            ...
            if (isPopup && window.opener) {
                window.opener.postMessage(
                    {
                        type: "UPDATE_GUEST",
                        guestId: guestId, // ✅ 업데이트된 손님 ID 전송
                    },
                    "*"
                );
            }
            ...
            <div className="page__manage__header">
                <div className="flex flex-row justift-between">
                    ...
                    <div className="page__manage__header__top">
                        <Label className={"text-3xl font-bold"}>손님 등록</Label>
                        <Label className={"text-xl text-gray-700 pt-3"}>{`등록번호 : ${guestId}`}</Label>
                    </div>
