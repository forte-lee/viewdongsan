 ...
 * @returns 방 개수 (1=원룸, 2=투룸, 3=쓰리룸, 4이상=4룸이상), 없으면 undefined
 */
...
/**
 * 평균 사이즈 점수 계산
 * 조건: 같은 구(區) + ON+OFF 모두 포함 + 아파트 또는 오피스텔 타입 + 같은 방 개수(룸 개수)
 * 
 * @param currentProperty 현재 매물
 * @param allProperties 전체 매물 목록
 * @returns 평균 사이즈 점수, 계산 불가능한 경우 undefined
 */
export function calculateAverageSizeScore(
    ...
    if (needsRoomCountCheck && !currentRoomCount) {
        return undefined; // 방 개수가 없으면 계산 불가
    }
    ...
    const filteredProperties = allProperties.filter((property) => {
        ...
        // 4. 같은 방 개수(룸 개수) - 상가, 토지, 건물, 사무실은 제외
        ...
        // 5. 사이즈 점수 계산 가능한 매물 (calculateSizeScore가 undefined가 아니고 0이 아닌 것)
    ...
    // 사이즈는 참고면적(평)을 그대로 사용하므로 소수점 첫째 자리까지 반환
